        ...
        match second_token {
            ...
            "cemb.fromstack" => {
                if let Some(&stack_element) = tokens.get(2) {
                    let stack_element: usize =
                        stack_element.parse().expect("Failed to convert to Usize");
            ...
            _ => {
                if let Some(&last_element) = tokens.last() {
                    if second_token.starts_with("'") && last_element.ends_with("'") {
                        ...
                        //printline 2 + 1
                        if tokens.len() > 3 {
                            if let Ok(number_integer_1) =
                                (tokens.get(1).expect("Failed to receive or parse")).parse::<i64>()
                            {
                                if let Ok(number_integer_2) =
                                    (tokens.get(3).expect("Failed to receive or parse"))
                                        .parse::<i64>()
                                {
                                    if let Some(&operator) = tokens.get(2) {
                                        match operator {
                                            ...
                                            _ => {
                                                println!(
                                                    "{}: {}",
                                                    "Error: Not a valid operator ".red(),
                                                    operator
                                                );
                                            }
                                        }
                                    ...
                                    println!(
                                        "{} {}",
                                        "Not an integer:".red(),
                                        tokens.get(1).expect("Failed to receive it")
                                    );
                                }
                            } else if let Ok(number_float_1) =
                                (tokens.get(1).expect("Failed to receive or parse")).parse::<f64>()
                            {
                                if let Ok(number_float_2) =
                                    (tokens.get(3).expect("Failed to receive or parse"))
                                        .parse::<f64>()
                                {
                                    if let Some(&operator) = tokens.get(2) {
                                ...
                                println!(
                                    "{}: {}",
                                    "Error: Not a valid piece of integer nor float ".red(),
                                    (tokens.get(1).expect("Failed to receive it"))
                                );
                                std::process::exit(1)
                            }
                            ...
                            for element in &stack {
                                let metadata_array: Vec<String> =
                                    element.split_whitespace().map(|s| s.to_string()).collect();
